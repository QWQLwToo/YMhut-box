...
class UIManager {
    constructor() {
        this.modularTools = {
            ...
            'qr-code-generator': {
                ...
                launchType: 'inline' // <-- 设为内联
            },
            // [新增] 敏感词检测
            'profanity-check': {
                name: '敏感词检测',
                icon: 'fas fa-shield-alt',
                description: '检测文本中的敏感词和违禁词',
            ...
            'dns-query': {
                ...
                description: '查询域名的DNS A, MX, TXT等记录',
                classId: 'dns-query',
                launchType: 'window',
                view: 'view-tool'
            },
    ...
    _bindToolboxPageEvents() {
        ...
        const slider = document.getElementById('toolbox-active-slider'); // [新增] 获取滑块
        ...
        if (defaultTab) {
            ...
            moveSlider(defaultTab); // [新增] 初始定位滑块
        }
    ...
    _filterAndRenderTools() {
        ...
        const animationDuration = 200; // 动画时长(毫秒)，必须与 CSS 中的 0.2s 匹配
        ...
        grid.querySelectorAll('.tool-card').forEach(card => {
            card.addEventListener('click', (e) => {
                if (card.classList.contains('disabled')) {
                    ...
                    // [BUG 修复]
                    // 错误：uiManager.showNotification('工具维护中', message, 'info');
                    // 正确：
    ...
    launchModularTool(toolId) {
        ...
        if (toolInfo.launchType === 'window') {
            ...
            // 我们请求主进程打开一个新窗口，
            // 传入 "视图" 名称 (e.g., 'view-browser') 和 "工具ID" (e.g., 'my-new-tool')
            window.electronAPI.openToolWindow(toolInfo.view, toolId, currentTheme);

            // 2. 默认：内联启动 (inline)
        } else {
            const ToolClass = toolRegistry[toolInfo.classId]; // <-- 从注册表获取类
            if (!ToolClass) {
    ...
    renderSubcategories(categoryId, page = 1) {
        ...
        // [重要] _loadAndCountThumbnails 现在只会找到未被禁用的图片
    ...
    showNotification(title, message, type = 'success') {
        ...
        if (!container) {
            console.error('错误：未在HTML中找到 #toast-container 元素，无法显示通知。');
            return;
        }
    ...
    showPrompt(options, callback) {
        ...
        const onCancel = () => {
            if (callback) callback(null); // 传递 null 表示取消
            cleanup();
        }
